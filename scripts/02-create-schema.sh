...
PGPASSWORD=$DB_PASSWORD psql \
    ...
    -f $OSMOSIS_SCRIPTS_DIR/pgsnapshot_schema_0.6.sql

log "Applying action schema"
PGPASSWORD=$DB_PASSWORD psql \
    -h "$DB_HOST" -U "$DB_USER" -d "$DB_NAME" \
    -f $OSMOSIS_SCRIPTS_DIR/pgsnapshot_schema_0.6_action.sql

log "Applying bbox schema"
PGPASSWORD=$DB_PASSWORD psql \
    -h "$DB_HOST" -U "$DB_USER" -d "$DB_NAME" \
    -f $OSMOSIS_SCRIPTS_DIR/pgsnapshot_schema_0.6_bbox.sql

log "Applying linestring schema"
PGPASSWORD=$DB_PASSWORD psql \
    -h "$DB_HOST" -U "$DB_USER" -d "$DB_NAME" \
    -f $OSMOSIS_SCRIPTS_DIR/pgsnapshot_schema_0.6_linestring.sql
